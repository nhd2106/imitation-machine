...
For the opt-in installed OpenCode integration lane in this repo checkout that scaffolds one `docs-review` temp repo and runs a bounded 3-turn session (`opencode run --print-logs`, then two `opencode run --print-logs --continue` calls in the same repo), run `bun run test:opencode:installed` (or `bash tests/opencode/run-tests.sh installed`). The happy path requires fresh `review-spec`, `review-quality`, and `bun test` evidence after each later write. Set `OPENCODE_INSTALLED_LIVE=1` to execute the real session; otherwise it skips cleanly.
