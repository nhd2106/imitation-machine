...
const DEEPENED_FIXTURE_EXPECTATIONS = {
  ...
  "tests/skill-triggering/systematic-debugging-prompts.md": [
    ...
    ["feedback loop", "pass/fail signal"],
    ["flaky repro rate", "higher reproduction rate"],
    ["ranked", "rank"],
    ...
    ["regression seam", "test seam"],
    ["no tracker-publishing shortcut", "tracker publishing"],
    ["requirements intake", "requirements brief", "prd"],
    ["broad refactor", "broad refactors"],
    ["evidence", "narrow"],
  ...
  "tests/explicit-skill-requests/systematic-debugging-prompts.md": [
    ...
    ["feedback loop", "pass/fail signal"],
    ["flaky repro rate", "reproduction rate"],
...
const DISTINCT_SCENARIO_EXPECTATIONS = {
  ...
  "tests/skill-triggering/systematic-debugging-prompts.md": [
    [/fails sometimes|intermittent|flaky/, /feedback loop|pass\/fail signal/, /flaky repro rate|reproduction rate/],
    [/hypothesis log/, /ranked|rank/, /falsifiable|prediction/],
    [/minimize|minimal|smallest/, /regression seam|test seam/, /original symptom/],
    [/instrumentation|instrument/, /prediction-based probe|prediction/, /one variable at a time|one variable/],
    [/patch three files|random fixes/, /debug systematically|slow this down/],
    [/tracker publishing|tracker update/, /no tracker-publishing shortcut|no tracker/, /separate approved workflow|separate workflow/],
    [/requirements intake|requirements brief|prd/, /not requirements intake|do not treat systematic-debugging as requirements intake/, /separate approval|separate approved workflow/],
    [/broad refactor|broad refactors/, /not permission|do not use debugging as permission/, /root-cause evidence|root cause.*evidence|follow-up/],
  ],
  "tests/explicit-skill-requests/systematic-debugging-prompts.md": [
    [/reproduce|reproducible/, /feedback loop|pass\/fail signal/, /flaky repro rate|reproduction rate/],
    [/hypothesis log|logged hypotheses/, /ranked|rank/, /falsifiable|prediction/],
    [/minimize|minimal|smallest/, /regression seam|test seam/, /original symptom verification|original symptom/],
    [/one variable at a time|one variable/, /prediction-based probe|prediction/, /confirm or kill|confirm\/kill|kill/],
    [/random patches/, /evidence/, /likely cause|narrows the cause/],
    [/tracker publishing|tracker update/, /no tracker|tracker-publishing boundary/, /separate approved workflow|separate workflow/],
    [/requirements intake|requirements brief|prd/, /not requirements intake|do not treat systematic-debugging as requirements intake/, /separate approval|separate approved workflow/],
    [/broad refactor|broad refactors/, /not permission|do not use debugging as permission/, /root-cause evidence|root cause.*evidence|follow-up/],
  ],
  "tests/skill-triggering/grill-me-prompts.md": [
...
const EXPECTED_MULTI_TURN_PROGRESSIONS = {
  ...
  "tests/multi-turn-workflows/systematic-debugging-to-fix.md": [
    ["`systematic-debugging`", "production-only", "feedback loop", "pass/fail signal", "flaky repro rate"],
    ["hypothesis log", "ranked falsifiable hypotheses", "prediction-based probe", "one variable"],
    ["regression seam", "original symptom verification", "temporary instrumentation cleanup", "bounded fix plan"],
    ["tdd", "failing regression", "root-cause evidence", "no tracker-publishing shortcut"],
  ],
  "tests/multi-turn-workflows/repo-to-scope-to-verify.md": [
