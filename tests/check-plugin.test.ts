...
describe("check-plugin command", () => {
  test("passes when required plugin files and manifest paths exist", async () => {
    ...
    await Bun.write(join(root, "agents/coder.md"), "---\nname: coder\ndescription: coder agent\n---\n");
    await Bun.write(
  ...
  test("warns when agents directory is missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "agentic-check-plugin-no-agents-"));
    await mkdir(join(root, ".opencode/plugins"), { recursive: true });
    await mkdir(join(root, ".claude-plugin"), { recursive: true });
    await mkdir(join(root, "skills/using-agentic"), { recursive: true });
    await mkdir(join(root, "bin"), { recursive: true });
    ...
    await Bun.write(
      join(root, ".claude-plugin/plugin.json"),
      JSON.stringify({ name: "imitation-machine", version: "0.1.0", author: { name: "duoc95" } }),
    );
    await Bun.write(
      join(root, "package.json"),
      JSON.stringify({
    ...
    expect(
      report.checks.some(
        (check) => check.id === "claude:agents-dir" && check.severity === "warn",
      ),
    ).toBe(true);
  });

  test("passes hooks check when hooks file is declared and exists", async () => {
    const root = await mkdtemp(join(tmpdir(), "agentic-check-plugin-hooks-"));
    await mkdir(join(root, ".opencode/plugins"), { recursive: true });
    await mkdir(join(root, ".claude-plugin/hooks"), { recursive: true });
    await mkdir(join(root, "skills/using-agentic"), { recursive: true });
    await mkdir(join(root, "agents"), { recursive: true });
    await mkdir(join(root, "bin"), { recursive: true });
    await mkdir(join(root, ".opencode"), { recursive: true });
    ...
    await Bun.write(join(root, "agents/coder.md"), "---\nname: coder\ndescription: coder\n---\n");
    await Bun.write(
      join(root, ".claude-plugin/hooks/hooks.json"),
    ...
    expect(
      report.checks.some(
        (check) => check.id === "claude:hooks-file" && check.severity === "pass",
      ),
    ).toBe(true);
  });

  test("fails when hooks file is declared but missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "agentic-check-plugin-hooks-missing-"));
    await mkdir(join(root, ".opencode/plugins"), { recursive: true });
    await mkdir(join(root, ".claude-plugin"), { recursive: true });
    await mkdir(join(root, "skills/using-agentic"), { recursive: true });
    await mkdir(join(root, "bin"), { recursive: true });
    ...
    expect(
      report.checks.some(
        (check) => check.id === "claude:hooks-file" && check.severity === "fail",
      ),
    ).toBe(true);
  });
});
