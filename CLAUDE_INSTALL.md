...
- expanded workflow skills appear, including `systematic-debugging`, `dispatching-parallel-agents`, `executing-plans`, `finishing-a-development-branch`, and `receiving-code-review`
...
Typical follow-on workflow choices now include `systematic-debugging` for stubborn failures, `dispatching-parallel-agents` for safe concurrency, `executing-plans` for approved-plan direct execution, `receiving-code-review` for reply/fix loops, and `finishing-a-development-branch` for final handoff cleanup.
