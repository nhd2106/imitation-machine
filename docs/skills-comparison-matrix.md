...
| `tdd` | `test-driven-development` | Comparable | Partial | Partial | has multi-turn workflow depth via `using-agentic -> tdd -> verify`, but pressure-scenario evals are still lighter than the best-covered skills | `tests/multi-turn-workflows/using-agentic-to-tdd-to-verify.md` |
...
| `receiving-code-review` | `receiving-code-review` | Comparable | Partial | Partial | has multi-turn reviewer-feedback response coverage, but defer-vs-fix scenarios are still limited | `tests/multi-turn-workflows/requesting-to-receiving-code-review.md` |
...
3. Deepen the remaining thinner packages (`repo`, `adr`, `commit`, `executing-plans`, `dispatching-parallel-agents`, `review-security`, `systematic-debugging`) only when the extra end-to-end coverage is meaningfully distinct.
