...
| `plan` | `writing-plans` | Partial | Partial | Partial | still thinner than the richest plan packages, but now includes serial + grouped example artifacts and a bounded `repo -> plan -> subagent-driven-development` handoff | `tests/multi-turn-workflows/repo-to-plan-to-subagent-driven-development.md` |
