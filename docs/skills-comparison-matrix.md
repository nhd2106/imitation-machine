...
- The remaining meaningful product gaps from the latest deep compare are `prototype`, `architecture-deepening`, systematic-debugging depth, and `requirements-brief` / `issue-slicing` enrichment.
...
The current quality/delegation/coding-control wave closed the TDD depth, receiving-review depth, executing-plans isolation, QA/persona drift closed, final holistic review-final gaps, and the OpenCode-only dangerous-git guardrails for the local skill/agent package. Remaining work is selective: `prototype`, `architecture-deepening`, systematic-debugging depth, and `requirements-brief` / `issue-slicing` enrichment.
...
3. Deepen the remaining thinner packages (`repo`, `adr`, `commit`, `dispatching-parallel-agents`, `review-security`, `systematic-debugging`, `review-final`) only when the extra end-to-end coverage is meaningfully distinct.
