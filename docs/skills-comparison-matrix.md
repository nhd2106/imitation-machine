...
| `repo` | no strong direct counterpart | Unique | Partial | Partial | trigger and explicit-request pressure coverage now include base-branch uncertainty, affected packages, transitive dependency impact, and scoped-vs-full verification justification, but broader cross-workspace execution examples are still limited | `tests/explicit-skill-requests/repo-prompts.md` |
